...
CERT_PASS='MDzRi4ay?mt/N7#Y-@M1x'
# Using $HOME instead of ~ to avoid path resolution issues in scripts
ZIP_FILE="./DCWC_Resources_070926.zip"
...
do
    ...
    if [ $? -eq 0 ]; then
        ...
        sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=no "$REMOTE_USER@$IP" "
            ...
            unzip -o ~/DCWC_Resources_070926.zip -d ~/update_ios_temp;
            cd ~/update_ios_temp;
            ...
            unzip -o WB_Dist_26-27.zip -d ./;
            
            # Import the .p12 Certificate into the keychain
            security import WB_Dist_26-27.p12 -k ~/Library/Keychains/login.keychain-db -P '$CERT_PASS' -T /usr/bin/codesign;
            ...
            cp DCWC_mobileprovisions/DCWC_AdHoc.mobileprovision ~/Library/MobileDevice/Provisioning\ Profiles/;
            cp DCWC_mobileprovisions/DCWC_AppStore.mobileprovision ~/Library/MobileDevice/Provisioning\ Profiles/;
            
            # Clean up temporary files to keep the server tidy
            # rm -rf ~/update_ios_temp ~/DCWC_Resources_070926.zip;
